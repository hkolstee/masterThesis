...
class SequentialTabularQLearning:
    ...
    def update_table(self, obs, actions, rewards, next_obs, agent_idx):
        ...
        if agent_idx < (self.nr_agents - 1):
            ...
            td_target = maxQ_next_agent
            td_error = td_target - Q_taken_action
            # update
            self.q_tables[agent_idx][tuple(obs)][tuple(actions[:agent_idx + 1])] += ((agent_idx + 1) / (self.nr_agents)) * self.lr * td_error
        else:
            ...
            td_target = np.mean(rewards) + self.gamma * maxQ_next_obs
            td_error = td_target - Q_taken_action
            # update
