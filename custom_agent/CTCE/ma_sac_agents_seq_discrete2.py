...
class Agents:
    ...
    def __init__(self, 
        ...
        self.critic1 = Critic(lr = lr_critic,
                              obs_size = self.obs_size_global + sum([act_space.n for act_space in self.env.action_space]) + self.agent_ids.shape[0],
                              layer_sizes = layer_sizes)
    ...
    def learn(self):
        ...
        replay_act = [functional.one_hot(torch.tensor(actions, dtype = torch.int64)).squeeze().float().to(self.device) for actions in replay_act_list]
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            next_obs_set = torch.cat(obs, dim = 1)
            ...
            for agent_idx in range(self.nr_agents):
                ...
                # seq_pi_input = torch.cat([obs_set, self.create_padded_action_input(seq_a_im1, agent_idx)], dim = 1)
                ...
                #         seq_pi_input = torch.cat([obs_set, self.create_padded_action_input(seq_a_i, agent_idx)], dim = 1)
                ...
                #         seq_pi_input = torch.cat([next_obs_set, self.create_padded_action_input(torch.empty((replay_act[0].shape[0], 0)), 0)], dim = 1)
                #         # get action
                ...
                seq_Q_input = self.create_padded_action_input(seq_a_im1, agent_idx)
                ...
                with torch.no_grad():
                    ...
                    if agent_idx < (self.nr_agents - 1):
                        seq_Q_targ_input = self.create_padded_action_input(seq_a_i, agent_idx + 1)
                    ...
                    # But, we take the first critic in the sequence to calculate this target.
                        ...
                        seq_Q_targ_input = self.create_padded_action_input(torch.empty((replay_act[0].shape[0], 0)), 0)
                        ...
                        # adapted bellman target for discrete actions
                        print(rewards.shape)
                        bellman = torch.mean(rewards, dim = 1) + self.gamma * (1 - dones[0]) * q_targ
