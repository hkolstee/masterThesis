...
class Agent:
    ...
    def __init__(self, 
        ...
        self.actor = Actor(lr_actor, 6, env.action_space.shape[0], env.action_space.low, env.action_space.high, layer_sizes)
        self.critic1 = Critic(lr_critic, 6, env.action_space.shape[0], layer_sizes)
        self.critic2 = Critic(lr_critic, 6, env.action_space.shape[0], layer_sizes)
    ...
    def learn(self):
        ...
        latent_obs, decoded_obs = self.autoencoder.forward(obs)
        ae_loss = functional.mse_loss(decoded_obs, obs)
        ae_loss.backward()
        self.autoencoder.optimizer.step()
        # next obs
        self.autoencoder.optimizer.zero_grad()
        latent_next_obs, decoded_next_obs = self.autoencoder.forward(obs)
        ae_loss = functional.mse_loss(decoded_next_obs, next_obs)
        ae_loss.backward()
        self.autoencoder.optimizer.step()
        # detach
        latent_obs = latent_obs.detach()
        latent_next_obs = latent_next_obs.detach()

        # reset actor gradient
        self.actor.optimizer.zero_grad()

        # compute current policy action for pre-transition observation
        policy_act_prev_obs, log_prob_prev_obs = self.actor.normal_distr_sample(latent_obs)
        ...
        q1_buffer = self.critic1.forward(latent_obs, replay_act)
        q2_buffer = self.critic2.forward(latent_obs, replay_act)
        ...
        with torch.no_grad():
            ...
            policy_act_next_obs, log_prob_next_obs = self.actor.normal_distr_sample(latent_next_obs)
            # policy_act_next_obs, log_prob_next_obs = self.actor.normal_distr_sample(next_obs)

            # target q values
            q1_policy_targ = self.critic1_targ.forward(latent_next_obs, policy_act_next_obs)
            q2_policy_targ = self.critic2_targ.forward(latent_next_obs, policy_act_next_obs)
        ...
        # compute Q-values
        q1_policy = self.critic1.forward(latent_obs, policy_act_prev_obs)
        q2_policy = self.critic2.forward(latent_obs, policy_act_prev_obs)
