...
class Agents:
    ...
    def __init__(self, 
        ...
        self.agent_ids = F.one_hot(torch.tensor(range(self.nr_agents))).float()
        # create premade batch sized agent id tensors for insertion into batch of padded input
        ...
        #           Requires homogenous agent action spaces. No one hot ID required as extra sequential actions create disjoint obs spaces, but 
        #           used because padding continuous actions is problematic with choosing values.
        ...
        actor_input_size = obs_size_global + sum([size[0] for size in act_sizes[:-1]]) + self.agent_ids.shape[0]
        ...
        self.critic1 = Critic(lr_critic,
                              ...
                              sum([size[0] for size in act_sizes]),
                              layer_sizes)
