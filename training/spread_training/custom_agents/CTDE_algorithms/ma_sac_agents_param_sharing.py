...
class Agents:
    ...
    def __init__(self, 
        ...
        self.actor = Actor(lr_actor, 
                           obs_sizes[0] + self.agent_ids[0].shape[0], 
    ...
    def learn(self):
        ...
        with torch.no_grad():
            if not self.global_observations:
                ...
                obs_set = observations[0]
                next_obs_set = next_observations[0]
        replay_act_set = torch.cat(replay_actions, dim = 1)

        # create param sharing input for each agent (add onehot id)
        template_tensor = torch.tensor((self.batch_size, self.actor.input_size)).to(self.device)
        input_tensors_actor_obs = []
        input_tensors_actor_next_obs = []
        # add onehot ids
        with torch.no_grad():
            for idx in range(self.nr_agents):
                actor_obs = template_tensor.copy()
                actor_next_obs = template_tensor.copy()
                # add observation data
                actor_obs[:, 0 : observations[idx].shape[1]] = observations[idx]
                actor_next_obs[:, 0 : observations[idx].shape[1]] = next_observations[idx]
                # add one hot id
                actor_obs[:,  -self.agent_id_tensors[idx].shape[1] :] = self.agent_id_tensors[idx]
                actor_next_obs[:,  -self.agent_id_tensors[idx].shape[1] :] = self.agent_id_tensors[idx]
                # add to list
                input_tensors_actor_obs.append(actor_obs)
                input_tensors_actor_next_obs.append(actor_next_obs)
        ...
        with torch.no_grad():
            policy_act_next_obs, log_prob_next_obs = \
                zip(*[self.actor.normal_distr_sample(next_obs) for next_obs in input_tensors_actor_next_obs])
            policy_act_next_obs_set = torch.cat(policy_act_next_obs, axis = 1)
                
        # with grad 
        policy_act_obs, log_prob_obs = \
            zip(*[actor.normal_distr_sample(obs) for (actor, obs) in zip(self.actors, input_tensors_actor_obs)])
        policy_act_obs_set = torch.cat(policy_act_obs, axis = 1)
        ...
        for agent_idx in range(self.nr_agents):
            alpha_loss = -(self.log_alphas[agent_idx].exp() * (log_prob_obs[agent_idx].detach() + self.entropy_targs[agent_idx])).mean()
        ...
        # compute Q-values
        q1_policy = self.critic1.forward(obs_set, policy_act_obs_set)
        q2_policy = self.critic2.forward(obs_set, policy_act_obs_set)
        ...
        for agent_idx in range(self.nr_agents):
            ...
            loss_policy = (alphas[agent_idx] * log_prob_obs[agent_idx] - q_policy).mean()
            ...
            log_prob_list.append(log_prob_obs[agent_idx].cpu().detach().numpy().mean())
    ...
    def train(self, nr_steps, max_episode_len = -1, warmup_steps = 10000, learn_delay = 1000, learn_freq = 50, learn_weight = 50, 
        ...
        for step in range(nr_steps):
            ...
            # add agent ids to observations before storing
            # new_obs = [o + agent_id for (o, agent_id) in zip(obs, self.agent_ids.numpy())]
            # new_next_obs = [o + agent_id for (o, agent_id) in zip(next_obs, self.agent_ids.numpy())]

            # add transition to buffer
            # self.replay_buffer.add_transition(new_obs, action, reward, new_next_obs, done)
