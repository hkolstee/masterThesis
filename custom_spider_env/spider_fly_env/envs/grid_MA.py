...
class SpiderFlyEnvMA(gym.Env):
    ...
    def __init__(self, size = 5, spiders = 4, flies = 1, max_timesteps = 1000, render_mode = None):
        ...
        for agent_idx in range(self.nr_spiders):
            ...
            self.observation_space.append(spaces.Box(0, self.size - 1, (2 * self.nr_spiders + 2 * self.nr_flies,), dtype = np.int32))
    ...
    def _get_obs(self):
        ...
        return [np.array(np.append(self._spider_locations, self._fly_locations)) for _ in self.possible_agents]
    ...
    def step(self, actions):
        ...
        if any(flies_caught):
            rewards = [1 for _ in self.possible_agents]
            # terminals = {a: True for a in self.possible_agents}
        else:
            rewards = [0.001 * (rew) for rew in spiders_rew]

        # get observation
        observations = self._get_obs()
        # infos
        infos = [{} for _ in self.possible_agents]
        # terminals
        terminals = [False for a in self.possible_agents]
        # truncations
        if self.max_steps == self.timestep:
            truncations = [True for _ in self.possible_agents]
            self.timestep = 0
        else:
            truncations = [False for _ in self.possible_agents]
