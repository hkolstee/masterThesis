...
class SpiderFlyEnv(gym.Env):
    ...
    def __init__(self, size = 5, spiders = 4, render_mode = None, multiagent = False):
        super().__init__()
        self.size = size
    ...
    def step(self, actions):
        ...
        if terminal:
            ...
            reward = occupied_sides

        # get observation
        observations = self._get_obs()

        if self.multiagent:
            observations = [observations for _ in range(self.nr_spiders)]
            reward = [reward for _ in range(self.nr_spiders)]
